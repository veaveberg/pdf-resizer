...
struct PDFThumbnailView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSImageView {
        ...
        // Use the enhanced thread-safe thumbnail generator method with all parameters
        if let thumbnail = ContentView.shared.generatePDFThumbnail(
            for: url,
            page: currentPage,
    ...
    func updateNSView(_ imageView: NSImageView, context: Context) {
        // Update thumbnail using the thread-safe method with all parameters
        if let thumbnail = ContentView.shared.generatePDFThumbnail(
            for: url,
            page: currentPage,
            bleedTrimAmount: bleedTrimAmount,
    ...
    private func generateThumbnail(from page: PDFPage, size: NSSize, bleedTrimAmount: CGFloat, resizeMode: ResizeMode, targetSize: CGSize?, showFillPreview: Bool) -> NSImage {
        ...
        if let ctx = NSGraphicsContext.current?.cgContext {
            ...
            // Apply trim overlay if trim is active
            if bleedTrimAmount > 0 {
                ctx.saveGState()
            ...
            if totalPages > 1 {
                ...
                let controlX: CGFloat = (size.width - controlWidth) / 2
                ...
                // Draw page text
                let pageText = "\(currentPage + 1) of \(totalPages)"
...
struct ContentView: View {
    ...
    func generatePDFThumbnail(for url: URL, page pageIndex: Int, bleedTrimAmount: CGFloat = 0, resizeMode: ResizeMode = .fillSize, targetSize: CGSize? = nil, showFillPreview: Bool = false) -> NSImage? {
        // Create a dispatch group to ensure we wait for the result
        ...
        // Use userInteractive QoS to match UI thread's priority
        DispatchQueue.global(qos: .userInteractive).async {
